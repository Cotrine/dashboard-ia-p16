...
opcion = st.sidebar.radio("Elige una función:", ["Analizador de Sentimiento", "Chatbot General", "Ver Historial","Resumidor de PDFs", "Segmentación de Clientes"])
    ...
    if st.button("Analizar"):
        if texto:
            with st.spinner("La RTX 2000 está pensando..."):
                try:
                    res = requests.post(f"{https://uninhibited-guardlike-stuart.ngrok-free.dev}/analizar", json={"texto": texto})
                    datos = res.json()
        ...
        if pregunta:
            with st.spinner("Escribiendo..."):
                try:
                    ...
                    res = requests.post(f"{https://uninhibited-guardlike-stuart.ngrok-free.dev}/chat", json={"pregunta": pregunta})
                    datos = res.json()
...
# --- OPCIÓN 3: HISTORIAL (Las Gráficas) ---
    ...
    if st.button("Cargar Datos"):
        try:
            res = requests.get(f"{https://uninhibited-guardlike-stuart.ngrok-free.dev}/historial")
        ...
        if st.button("Generar Resumen"):
            with st.spinner("Leyendo y analizando..."):
                try:
                    ...
                    res = requests.post(f"{https://uninhibited-guardlike-stuart.ngrok-free.dev}/resumir", json={"texto_largo": texto_completo})
